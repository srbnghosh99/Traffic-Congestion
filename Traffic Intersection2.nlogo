...
to setup
  ...
  ask patches [
    ...
    set crosswalk1 patches with
    [(abs pxcor <= 10 and pxcor >= 9) and abs pycor <= 3]
    set crosswalk2 patches with
    [(abs pycor <= 10 and pycor >= 9) and abs pxcor <= 3]

   ask crosswalk1 [ set pcolor white ]
    ask crosswalk2 [ set pcolor white ]
...
to go
  ...
  ask pedestrians [ move
   find-nearest-neighbour
    set  car-speed-nearest [speed] of nearest-neighbour

  ]
  check-for-collisions
  make-new-car freq-north 0 min-pycor 0
  make-new-car freq-east min-pxcor 0 90
  make-new-pedestrian freq-north 9 9 180
  make-new-pedestrian freq-east 8 9 270
...
to adjust-walk-speed

  ; calculate the minimum and maximum possible speed I could go
  let min-speed 0 ;max (list (speed - max-brake) 0)
  let max-speed 5 ;min (list (speed + max-accel) walk-speed)


  let target-speed max-speed ; aim to go as fast as possible
  let blocked-patch next-blocked-walk-patch
  if blocked-patch != nobody [
    ; if there is an obstacle ahead, reduce my speed
    ; until I'm sure I won't hit it on the next tick
    let space-ahead (distance blocked-patch - 1)
...
to-report next-blocked-walk-patch
    ; check all patches ahead until I find a blocked
  ; patch or I reach the end of the world

  let patch-to-check patch-here
  if patch-to-check = nobody  [fd -1]
  ask pedestrians [find-nearest-neighbour]
  while [ patch-to-check + 1 != nobody and not is-hindered?  patch-to-check and [speed] of nearest-neighbour = 2 ] [
    set patch-to-check patch-ahead ((distance patch-to-check) + 1)
  ]
  ; report the blocked patch or nobody if I didn't find any
  report patch-to-check
...
to find-nearest-neighbour
  set nearest-neighbour  min-one-of (other cars) [distance myself]
end
to-report is-blocked? [ target-patch ] ; turtle reporter
  report
    any? other pedestrians-on target-patch or
...
to-report is-hindered? [ target-patch ] ; turtle reporter
    let criticalgap patch 0 1
  report

    any? other cars-on target-patch or
    any? accidents-on target-patch or
    (;any? other cars-on criticalgap and
