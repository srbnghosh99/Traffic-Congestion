...
to setup
  ...
  ask patches [
    ...
    [(abs pxcor <= 5 and pxcor >= 4) and abs pycor <= 3]
    set crosswalk2 patches with
    [(abs pycor <= 5 and pycor >= 4) and abs pxcor <= 3]
...
to go
  ...
  make-new-pedestrian freq-north 4 9 180
  make-new-pedestrian freq-east 8 4 270
...
to make-new-pedestrian [ freq x y h ]
  if (random-float 200 < freq) and not any? turtles-on patch x y [
...
to adjust-walk-speed
  ...
  ; calculate the minimum and maximum possible speed I could go
  let min-speed  max (list (speed - max-brake) 0)
  let max-speed  min (list (speed + max-accel) walk-speed)
...
to-report next-blocked-walk-patch
  ...
  while [ patch-to-check != nobody and not is-hindered?  patch-to-check ] [
    set patch-to-check patch-ahead ((distance patch-to-check) + 1)
  ]
  ; report the blocked patch or nobody if I didn't find any
  report patch-to-check
...
to-report is-hindered? [ target-patch ] ; turtle reporter
   let criticalgap patches with
    [(abs pxcor <= 2 and pxcor >= 0) and abs pycor <= 2]
   report
  ;  any? other cars-on criticalgap or
    any? other cars-on target-patch or
    any? accidents-on target-patch and
